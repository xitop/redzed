...
.. class:: DataPoll(name, *, func, interval, retry_interval=None, validator=None, abort_after_failures=0, **block_kwargs)
  ...
  :param retry_interval:
    ...
    Optional *interval* value to be used after a failed attempt.

    - fixed value - *float|str|None*:
        When *retry_interval* is :const:`None` (default), the regular *interval* value
        will be used.

    - exponential backoff - *Sequence[float|str]*:
        A sequence of exactly two values
        - ``T_min`` and ``T_max`` - enables a so-called exponential backoff. The first retry
  ...
  **Initialization:**
    ...
    - :class:`!DataPoll` supports persistent state. The last known value can
      be thus restored on the following start. Use :class:`PersistentState` for that.
