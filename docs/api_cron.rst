...
- Implement a callback that the Cron will call at given times:
  ...
  .. method:: Block.rz_cron_event(now: datetime.time)
    ...
    :meth:`!Block.rz_cron_event` will be called when the Cron starts and then
    according to a schedule. It may be called also at other times, most notably when
...
.. method:: Cron.set_schedule(self, blk: Block, times_of_day: Collection[datetime.time]) -> None:
  ...
  Block's :meth:`Block.rz_cron_event` will be called at given times regardless of date.
